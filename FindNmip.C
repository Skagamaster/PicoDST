...
void FindNmip(int day=96){

  gStyle->SetOptStat(0);

  gStyle->SetTitleSize(0.2,"t");

  std::ofstream NmipFile("NmipConstants.txt",ofstream::out);

  double SingleMipPeakStartingValue,FitRangeLow,FitRangeHigh;
  FitRangeHigh               = 700.0;  // high edge of range along the x-axis
  double xlo(50),xhi(1500);
  ...
  func->SetParName(nMipsMax+1,"WIDbyMPV");  // this is the Landau WID/MPV, and should be about 0.15 for the EPD
  func->SetParameter(nMipsMax+1,0.15);
  func->SetParLimits(nMipsMax+1,0.13,0.175);  // usually I don't set limits, but this should be okay.
  // ------------------------------------------------------------------ 
  ...
  theCanvas->SaveAs("ADCspectra.pdf[");
  TFile* in = new TFile(Form("RootFiles/Day%03d.root",day),"READ");
  for (int ew=0; ew<2; ew++){
	...
	TLine* tnominal = new TLine(SingleMipPeakStartingValue,0,SingleMipPeakStartingValue,adc->GetMaximum());
	tnominal->SetLineColor(4);	  tnominal->Draw();
	double nMipFound = func->GetParameter(nMipsMax);
	NmipFile << Form("%d \t%d \t%d \t%d \t%f",day,ew,PP,TT,nMipFound);
	if (FitStatus!=0) NmipFile << "\t <---------------- Fit failed";
	else if (fabs(nMipFound-SingleMipPeakStartingValue)>15) NmipFile << "\t <---------- different from nominal";
	NmipFile << endl;
