...
void SkipRunAnalysisTest(const Char_t *inFile = 
  "/home/zandar/root/Root/Picodst/Data/st_physics_12126101_raw_3040006.picoDst.root") {
  ...
  for(Long64_t iEvent=0; iEvent<events2read; iEvent++) {
    ...
    picoTrack->nHits() < 15 ||
    TMath::Abs( picoTrack->pMom().pseudoRapidity() ) > 1.0 ||
    // picoTrack->pMom().pseudoRapidity() > -0.5 ||
    picoTrack->charge() > 0.0 ||
    // 0.2 > picoTrack->pPt() > 2.0 ||
    0.2 > picoTrack->pPt() > 1.0 ||
    nTracks < 5) {
  continue;
      } //for(Int_t iTrk=0; iTrk<nTracks; iTrk++)
      
      hTransvMomentum->Fill( picoTrack->pPt() );
      hPhiAngles->Fill(picoTrack->pMom().phi() );
      
      /// These are our variables for the reaction plane angle.
      Qx += (picoTrack->pPt())*cos(2*picoTrack->pMom().phi());
      Qy += (picoTrack->pPt())*sin(2*picoTrack->pMom().phi());
    }
    ...
    picoTrack->charge() > 0.0 ||
    // 0.2 > picoTrack->pPt() > 2.0 ||
    1.0 > picoTrack->pPt() > 2.0 ||
    nTracks < 5) {
  continue;
      } //for(Int_t iTrk=0; iTrk<nTracks; iTrk++)
      sumVar += cos(2*(picoTrack->pMom().phi()-PsiRP));
  ...
  string Tacos = "PROCESSED";
  Tacos.append(Queso);
  TString Tacos1 = Tacos;
  TString pathSave = "/home/zandar/Documents/PicoHists/";
